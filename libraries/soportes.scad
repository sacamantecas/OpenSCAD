...
module agujero_cilindrico_soportado(d, h, t, r, cascara=0, separa=0, anticolapso) { 
	...
	translate(t)
		rotate(r)
			difference() {
				cilindro(d=d, h=h+mp); // el agujero es ligeramente mas largo de lo que se pide!!
				difference() {
					cilindro(d=d-2*$gap_v_soporte, h=h+(separa?-separa:mp));					
					for ( signo = [ -1, 1 ])
						cubo([d/2, d, h+mp], t=[(signo * (d/2 + reduccion)/2), 0, 0]);
					if (abs(ac)<h) 
						intersection() {
							cubo(c=[(reduccion-2*espesor), d, h-2*cascara-separa+mp]);
